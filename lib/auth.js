...
function _expressSession(User, server) {
    return {
        ...
        login: async function (req, res, next) {
            let [err1, user] = await asyncf(User.findOne({email: req.body.email}).lean());
            if (err1) {
            ...
            let [err2, result] = await asyncf(bcrypt.compare(req.body.password, user.password));
            if (true === result) {
    ...
    return {
        ...
        login: async function(req, res, next) {
            ...
            if (true === result) {
                ...
                // res.cookie('token', token); Must store the token somewhere!
                res.locals.loginResult = 'Logged in';
                next();
            } else if (err2) {
                console.log('Error comparing passwords ' + err2);
...
function _passport(User, server) {
    ...
    return {
        ...
        sessionCheck: passport.authenticate('local'),
